...
export class AppConfigurator implements OnInit {
    shouldDisabled = computed(() => {
        return this.layoutService.isDarkTheme() || this.menuMode() === 'horizontal';
    })
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        ...
        const colors = ['blue']; // Using only one primary color
    ...
    onMenuThemeChange(theme: string) {
    ...
    onMenuModeChange(event: string) {
        this.layoutService.layoutConfig.update((prev) => ({
            ...prev,
            menuMode: event,
            menuTheme: event === 'horizontal' && this.darkTheme() ? 'dark' : event === 'horizontal' && !this.darkTheme() ? 'light' : this.menuTheme()
        }));
    }
